...
class quantum_nonlinear_obser_Pn(gym.Env):
    """Custom Environment that follows gym interface"""
    # metadata = {"render.modes": ["human", "rgb_array"], "video.frames_per_second": 50}

    def __init__(self,rundir,N,g_0,kappa,n_steps,n_episode,output_interval,n_envs,target_idx):
        #super(quantum, self).__init__()
        
        self.action_space = spaces.Box(-5, 5, shape=(2,), dtype=np.float32) # Delta, alpha_L
        self.observation_space = spaces.Box(0, np.inf, shape=(1,), dtype=np.float32) # Pn
        ...
        self.Pn_target= np.load('Pn_step_'+str(target_idx)+'.npy')
    ...
    def reset(self):
        ...
        observation = [0]#[fidelity(self.rho_AB_0, self.rho_target)] # Fidelity
    ...
    def step(self, action):
        ...
        result = smesolve(H_0 + self.H_coupling + H_F, self.rho_AB_0, times =    
                          np.linspace(0,self.dt,self.N_t), \
                              c_ops = [np.sqrt(self.kappa)*self.a, np.sqrt(self.gamma)*self.b], sc_ops = [],
        ...
        observation = [Pn]#Fidelity
        reward = -abs(Pn-self.Pn_target[self.numStep-2])#-abs(E_N_0-np.log(2))-abs(Pn+Mn-2.5)/10
        ...
        return observation, reward, self.done, {0:E_N_0,1:E_N_1,2:Mn,3:Fidelity}
