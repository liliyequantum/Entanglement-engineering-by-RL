...
class quantumEnvMeasureFilter(gym.Env):
    ...
    def __init__(self):
        ...
        self.measure_rate = np.load('./train_par/measure_rate.npy')
        self.n_steps = np.load('./train_par/n_steps.npy')
        self.n_envs = np.load('./train_par/n_envs.npy')
        self.n_episode = np.load('./train_par/n_episode.npy')
        self.mean_interval = np.load('./train_par/mean_interval.npy')
        self.ntraj = np.load('./train_par/ntraj.npy')
        self.filter_interval = np.load('./train_par/filter_interval.npy')
        self.gaussian_var = np.load('./train_par/gaussian_var.npy')
        self.output_interval = np.load('./train_par/output_interval.npy')
        self.mix_p = float(np.load('./train_par/mix_p.npy'))
        
        # parameter
        self.N = 2  # N Fock basis
        self.dt = 0.01 # WCM time step size
        self.N_t = 2
        self.kappa = 0.01 # photon decay rate
    ...
    def step(self, action):
        ...
        if self.numStep == self.n_steps:
           ...
           if self.numEpisode < self.n_episode:
               ...
               np.save('./dataImp/meanRewardEp.npy',self.meanRewardEp)
               np.save('./dataImp/EN0Ep.npy',self.EN0Ep)
               np.save('./dataImp/EN1Ep.npy',self.EN1Ep)
               
               if self.numEpisode in range(self.output_interval,self.n_episode,self.output_interval):
                     np.save('./dataImp/controlG'+str(self.numEpisode)+'.npy',self.controlG)
                     np.save('./dataImp/use_mean_current'+str(self.numEpisode)+'.npy',self.use_mean_current)
                     np.save('./dataImp/EN0_step'+str(self.numEpisode)+'.npy',self.EN0_step)
                     np.save('./dataImp/EN1_step'+str(self.numEpisode)+'.npy',self.EN1_step)

                     reward_env = np.zeros((self.numEpisode,self.n_envs))
                     for i in range(self.n_envs):
                         my_data = genfromtxt('./monitor/'+str(i)+'.monitor.csv', delimiter=',')
                     ...
                     np.save('./dataImp/reward_env_'+str(self.numEpisode)+'.npy',reward_env)
                     ...
                     plt.savefig('./picture/reward_envs_'+str(self.numEpisode)+'.png',bbox_inches='tight')
                     plt.close()
                     ...
                     plt.yticks(fontsize=16)
                     plt.xticks(fontsize=16)
                     plt.savefig('./picture/reward_'+str(self.numEpisode)+'.png',bbox_inches='tight')
                     plt.close()
                     ...
                     plt.yticks(fontsize=16)
                     plt.xticks(fontsize=16)
                     plt.savefig('./picture/E_N_'+str(self.numEpisode)+'.png',bbox_inches='tight')
                     plt.close()
                     ...
                     plt.yticks(fontsize=16)
                     plt.xticks(fontsize=16)
                     plt.savefig('./picture/EN_step_'+str(self.numEpisode)+'.png',bbox_inches='tight')
                     plt.close()
                     ...
                     plt.yticks(fontsize=16)
                     plt.xticks(fontsize=16)
                     plt.savefig('./picture/use_mean_current_'+str(self.numEpisode)+'.png',bbox_inches='tight')
                     plt.close()
                     ...
                     plt.yticks(fontsize=16)
                     plt.xticks(fontsize=16)
                     plt.savefig('./picture/controlG_'+str(self.numEpisode)+'.png',bbox_inches='tight')
                     plt.close()
