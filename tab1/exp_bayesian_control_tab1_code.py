...
def exp_bayesicanControl(N,kappa,gamma,num_time,dt,N_t,measure_rate,num_repeat,ntraj,dirname,lamb):
    ...
    Pn = np.zeros((num_time,num_repeat))
    ...
    for k in tqdm(range(num_repeat)):
        ...
        for i in range(num_time):
            ...
            H_coupling = exp_par * (b.dag()*a + b*a.dag())
            ...
            for j in range(ntraj):
                ...
                result = smesolve(H_0 + H_coupling, Qobj(rho0_traj[:,:,j],dims=[[N,N],[N,N]]), 
                                  ...
                                  e_ops = [pn], ntraj=1, solver="taylor1.5",tol = 1e-6,
...
def plot(dirname,lamb):
    ...
    plt.plot(times,Pn[:,num_repeat-1],linewidth=3,label='exp current')
    ...
    plt.plot(times,-lamb*(Pn[:,num_repeat-1] - 0.5),linewidth=3,label='G')
    plt.legend(fontsize=15)
    plt.xlabel('times',fontsize=18)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
...
if __name__ == '__main__':  
    ...
    dirname = './exp_bayesianControl/lambda_'+str(lamb)
    update_dir(dirname)
    ...
    # exp_bayesicanControl(N,kappa,gamma,num_time,dt,N_t,measure_rate,num_repeat,ntraj,dirname,lamb)
    plot(dirname,lamb)
