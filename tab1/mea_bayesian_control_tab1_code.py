...
def mea_bayesicanControl(N,kappa,gamma,num_time,dt,N_t,measure_rate,num_repeat,ntraj,\
                  mean_interval,filter_interval,gaussian_var,dirname,lamb):
    ...
    for k in tqdm(range(num_repeat)):
        ...
        for i in range(num_time):
            ...
            H_coupling = -lamb*(mea_par - 0.5) * (b.dag()*a + b*a.dag())
...
def plot(dirname,lamb):
    ...
    # plt.plot(times,-(mean_current-0.5),linewidth=3,label='mean G')
    plt.plot(times,-lamb*(use_mean_current[:,num_repeat-1]-0.5),linewidth=3,label='mean G')
    plt.legend(fontsize=15)
    plt.xlabel('times',fontsize=18)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
...
if __name__ == '__main__':  
    ...
    dirname = './mea_bayesianControl/lambda_'+str(lamb)
    update_dir(dirname)
    ...
    # mea_bayesicanControl(N,kappa,gamma,num_time,dt,N_t,measure_rate,num_repeat,ntraj,\
    #                   mean_interval,filter_interval,gaussian_var,dirname,lamb)
    plot(dirname,lamb)
